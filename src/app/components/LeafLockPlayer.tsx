...
type LeafLockPlayerProps = {
  ...
  /** When true (live room), join and play as soon as decks are ready — no Play tap. */
  autoStartLive?: boolean;
  /** Bump when user re-taps “Join live room” so auto-start re-runs after a pause. */
...
export default function LeafLockPlayer({
  ...
  const userPlaybackIntentRef = useRef<"playing" | "paused" | "stopped">(
    listenMode === "live" && autoStartLive ? "playing" : "stopped"
  );
  const userPausedLiveRef = useRef(false);
  ...
  /** When true, Live Radio uses real station stream; YouTube is muted. */
  const liveUsesStreamRef = useRef(false);

  const muteYouTubeDecks = useCallback(() => {
    try {
  ...
  const unmuteYouTubeDecks = useCallback(() => {
    try {
      const scaled = Math.max(1, volumeRef.current);
      playersRef.current.a?.unMute();
      playersRef.current.b?.unMute();
      playersRef.current.a?.setVolume(scaled);
      playersRef.current.b?.setVolume(scaled);
    } catch {
      // ignore
    }
  }, []);
  ...
  const applyDeckVolume = useCallback((deck: DeckId, gain: number) => {
    ...
    // Live + real stream: permanent <audio> is audible; mute YouTube.
    if (listenModeRef.current === "live" && liveUsesStreamRef.current) {
      try {
        player.setVolume(0);
        player.mute();
      ...
      if (userPlaybackIntentRef.current === "playing") {
        setLiveRadioVolume(volumeRef.current / 100, false);
      }
      return;
    }
  ...
  const finishBlend = useCallback(
    (outgoing: YTPlayer, outgoingDeck: DeckId, incomingDeck: DeckId, video: PlaylistVideo) => {
      ...
      liveBlendTargetIdRef.current = null;
      const shouldPlay = userPlaybackIntentRef.current === "playing";
      ...
      if (shouldPlay) {
        ...
        // Keep OS media host / session alive across the blend handoff.
      ...
      if (listenModeRef.current === "live") {
        ...
        if (upcoming && upcoming.id !== video.id) {
  ...
  }, [applyDeckVolume, isMuted, muteYouTubeDecks, volume]);
  ...
  const applyLiveStationTrack = useCallback(
    (
      ...
      // Already DJ-blending into this track — just lock revision, do not hard-cut.
      if (
  ...
  useEffect(() => {
    ...
    async function initPlayers() {
      try {
        ...
        if (listenModeRef.current === "live") {
          const shouldJoin =
            !userPausedLiveRef.current &&
            (readLiveWasPlaying() || autoStartLiveRef.current);
          if (shouldJoin) {
            userPlaybackIntentRef.current = "playing";
          ...
          void fetchLiveStation()
            .then((station) => {
              applyLiveStationTrackRef.current(station, {
                initialCue: !shouldJoin,
                resumePlayback: shouldJoin
              });
              if (shouldJoin) {
  ...
  const playLiveYouTube = useCallback(
    async (station: PublicStationPayload) => {
      ...
      // Host only — never audible music (YouTube is the song).
      startSilentMediaHost();
  ...
  const togglePlay = () => {
    ...
    // —— PRIVATE JUKEBOX: original YouTube engine + DJ blend ——
    const player = getActivePlayer();
  ...
  const bindMediaSession = useCallback(() => {
    ...
    try {
      // Live Radio: Media Session controls permanent stream element only.
      // Private jukebox: wraps YouTube player.
      ...
      // Live Radio: no next / previous / seek (server timeline).
  ...
  }, [getActivePlayer, muteYouTubeDecks, stopTimePolling, syncMediaBridge, unmuteYouTubeDecks]);

  const updateMediaSession = useCallback(
    (playing: boolean) => {
      if (!("mediaSession" in navigator)) return;

      const live = listenModeRef.current === "live";
      ...
      <div className="flex flex-col items-center gap-5 sm:gap-6 md:flex-row">
        ...
        >
          ...
          <button
            ...
            onClick={togglePlay}
            disabled={
              isLoadingPlaylist || (listenMode !== "live" && !playersReady)
            }
      ...
      <div className="mt-6 flex flex-col justify-between gap-y-4 text-sm sm:mt-8 md:flex-row md:items-center">
        <div className="text-zinc-500">
          ...
          {listenMode === "live" ? (
            ...
            <span className="mt-2 block text-xs text-zinc-500">
              Private jukebox uses YouTube (DJ Blend on). Install the app for best background play.
            </span>
          ) : null}
          {(isPlaying || isConnected) && !isLoadingPlaylist ? (
            <details className="mt-3 rounded-xl border border-zinc-800 bg-black/30 px-3 py-2 text-left">
              ...
              <p className="mt-2 text-xs text-zinc-500">
                Soft-switch apps or lock the screen — we keep Media Session controls alive. Fully
                force-closing Chrome still stops YouTube audio on Android (browser limit). Use
                lock-screen play/pause; close the tab to stop completely.
              </p>
              <button
                type="button"
                onClick={togglePlay}
